-- VIRALYA V1 — fonctions de la queue maison

-- Claim atomique d'un lot de jobs prêts (FOR UPDATE SKIP LOCKED → workers concurrents).
...
-- Enfile un job plan_day par avatar actif (appelé par pg_cron ou l'admin).
...
begin
  ...
  select 'plan_day', jsonb_build_object('avatar_id', a.id)
    from avatars a where a.status='active';
