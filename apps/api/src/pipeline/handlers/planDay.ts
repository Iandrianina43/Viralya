...
import { orgIdOfAvatar, recordUsage } from "../../domain/billing";
...
export async function planDay(job: JobRow): Promise<void> {
  ...
  if (orgId) await recordUsage({ orgId, avatarId, kind: "daily", estimatedUsd: 12 }); // réservation atomique (402 avant tout coût)
  for (const e of plan) {
