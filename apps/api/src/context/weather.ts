...
// Météo temps réel via Open-Meteo (gratuit, aucune clé). Non bloquant → null si échec.
...
const WMO: Record<number, string> = {
  ...
  80: "averses", 81: "averses", 82: "fortes averses", 95: "orage", 96: "orage grêle", 99: "orage violent",
};
...
async function geocode(city: string): Promise<{ lat: number; lon: number } | null> {
  ...
  const loc = f ? { lat: f.latitude, lon: f.longitude } : null;
...
export async function getWeather(city: string, timezone: string): Promise<WeatherInfo | null> {
  ...
  try {
    ...
    return { tempC: Math.round(t), description: WMO[data.current?.weather_code ?? -1] ?? "temps variable" };
  } catch (err) {
