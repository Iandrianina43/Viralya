...
### P2 — important
- ~~Fusion des relances : un rendu qui dépasse le délai de suivi (25 min) est marqué en échec alors que les clips peuvent encore arriver~~ — fait le 14 sept. : délai mesuré par plan depuis sa soumission (25 min hybride, 20 min par segment plan-séquence), dernière vérification puis annulation et resoumission du seul plan concerné ; plafond global 60 min qui monte les plans prêts au lieu d'échouer.
...
- ~~Grille du calendrier et panneau latéral sur mobile~~ (fait le 14 sept. : liste par jour sous 640 px, panneau en feuille du bas) ; ~~zone d'actions des cartes influenceur~~ (fait le 14 sept. : Bible · Journal · menu « Plus » pour contenu du jour, fiche, suppression).
- ~~Journalisation des requêtes (identifiant de requête, coût par job) ; `max_attempts` par type de job~~ — fait le 14 sept. : `x-request-id` (repris ou généré, renvoyé), ligne `http_request` par appel /api, `job_done` avec le coût cumulé du contenu, `MAX_ATTEMPTS_BY_TYPE` dans `queue/queue.ts` (publication 1, rendus payants 2, suivi 4).
...
- ~~Vocabulaire unifié (« influenceur » partout, plus « avatar »)~~ — fait le 14 sept. dans les textes visibles (pages, messages d'erreur, journal de production). Noms de fournisseurs retirés des écrans clients le 14 sept. (éditeur, création, studio, calendrier, assistant vlog) ; ils restent là où l'utilisateur choisit un moteur avec son prix (boutons de prise de parole, salle de production), dans les réglages admin et sur la page légale (sous-traitants).
